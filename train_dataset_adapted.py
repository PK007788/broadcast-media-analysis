"""
SSDLite320-MobileNetV3-Large DATASET-ADAPTED Training Script
============================================================
Experiment: Model B — Dataset-Adapted Detector (custom wide anchors)
Dataset:    news.coco overlay detection (259 images, 1 class)
Split:      Temporal-grouped (train=202, val=36, test=21)

Only difference from Model A: anchor ratios changed from [[2,3]]*6 to [[2,3,5,8]]*6
to better match the ultra-wide broadcast overlay distribution.
...
EXP_DIR = os.path.join(BASE_DIR, "runs", "ssdlite_mobilenetv3", "dataset_adapted")
...
CONFIG = {
    "experiment": "Model B — DATASET-ADAPTED",
    "model": "ssdlite320_mobilenet_v3_large",
    "backbone": "MobileNetV3-Large",
    "anchor_config": "custom [[2,3,5,8]]*6 (wide overlays)",
...
def main():
    ...
    # INSTANTIATE MODEL
    # --------------------------------------------------------
    print("\n=== MODEL INSTANTIATION (DATASET-ADAPTED) ===")
    model = ssdlite320_mobilenet_v3_large(
    ...
    # Replace anchor generator with custom wide-anchor configuration
    from torchvision.models.detection.anchor_utils import DefaultBoxGenerator
    from torchvision.models.detection.ssdlite import SSDLiteHead, SSDLiteClassificationHead
    import torch.nn as nn
    from functools import partial

    # Extract in_channels from the original head before replacing
    in_channels = [c[0][0].in_channels for c in model.head.classification_head.module_list]
    norm_layer = partial(nn.BatchNorm2d, eps=0.001, momentum=0.03)

    # Install custom anchor generator: [[2,3,5,8]]*6
    custom_anchor_gen = DefaultBoxGenerator(
        [[2, 3, 5, 8]] * 6,
        min_ratio=20,
        max_ratio=95
    )
    model.anchor_generator = custom_anchor_gen
    num_anchors = custom_anchor_gen.num_anchors_per_location()

    # Rebuild head to match the new anchor count
    model.head = SSDLiteHead(in_channels, num_anchors, NUM_CLASSES, norm_layer=norm_layer)

    # Verify custom anchors are active
    print(f"Custom anchor ratios: [[2,3,5,8]]*6")
    print(f"Anchors per location: {num_anchors}")
    assert num_anchors == [10, 10, 10, 10, 10, 10], f"Unexpected anchors: {num_anchors}"
    print("VERIFIED: Custom anchor generator is active.")
